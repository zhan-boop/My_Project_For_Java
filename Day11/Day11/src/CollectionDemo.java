...
public class CollectionDemo {
    ...
    public static void main(String[] args) {
        ...
        // 改：按下标修改
        servers.set(1, "web-02-updated");

        // 删：按下标删除
        servers.remove(3);  // 删掉 cache-01
        ...
        // 判断存在
        if (servers.contains("db-01")) {
            System.out.println("db-01 在线");
        }
        ...
        history.add(new String[]{"ls", "-la", "/var/log"});
        history.add(new String[]{"grep", "ERROR", "app.log"});
        history.add(new String[]{"tar", "-czf", "backup.tar.gz", "/data"});
        ...
        status.put("web-01", true);    // true=在线
        status.put("web-02", false);   // false=离线
        status.put("db-01", true);
        status.put("cache-01", true);

        // 查单台
        System.out.println("web-02 状态: " + (status.get("web-02") ? "在线" : "离线"));

        // 安全取值：不存在的 key 返回默认值
        String backupStatus = status.getOrDefault("backup-01", false) ? "在线" : "离线";
        System.out.println("backup-01 状态: " + backupStatus + " (默认)");

        // 遍历所有（entrySet = Python 的 .items()）
        System.out.println("\n全部状态:");
        ...
        String[] users = {"admin", "guest", "deployer", "hacker"};   // ★ contains 是 O(1)
        ...
        opLog.addFirst("10:05 重启 web-02");    // 最新操作加到头部
